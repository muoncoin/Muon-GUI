...
void serialize(ParentBlockSerializer& pbs, ISerializer& serializer) {
  ...
  if (pbs.m_hashingSerialization) {
    ...
    Crypto::tree_hash_from_branch(pbs.m_parentBlock.baseTransactioMUOanch.data(), pbs.m_parentBlock.baseTransactioMUOanch.size(), minerTxHash, 0, merkleRoot);
  ...
  if (serializer.type() == ISerializer::OUTPUT) {
    if (pbs.m_parentBlock.baseTransactioMUOanch.size() != branchSize) {
      throw std::runtime_error("Wrong miner transaction branch size");
    }
  } else {
    pbs.m_parentBlock.baseTransactioMUOanch.resize(branchSize);
  }

//  serializer(m_parentBlock.baseTransactioMUOanch, "baseTransactioMUOanch");
  //TODO: Make arrays with computable size! This code won't work with json serialization!
  for (Crypto::Hash& hash: pbs.m_parentBlock.baseTransactioMUOanch) {
    serializer(hash, "");
  }
  ...
  if (serializer.type() == ISerializer::OUTPUT) {
    if (mmTag.depth != pbs.m_parentBlock.blockchaiMUOanch.size()) {
      throw std::runtime_error("Blockchain branch size must be equal to merge mining tag depth");
    }
  } else {
    pbs.m_parentBlock.blockchaiMUOanch.resize(mmTag.depth);
  }

//  serializer(m_parentBlock.blockchaiMUOanch, "blockchaiMUOanch");
  //TODO: Make arrays with computable size! This code won't work with json serialization!
  for (Crypto::Hash& hash: pbs.m_parentBlock.blockchaiMUOanch) {
    serializer(hash, "");
  }
}
